...
const CORS_FIREBASE = /^https:\/\/lealteck-plataforma\.(web\.app|firebaseapp\.com)$/
app.use(cors({
  origin: (origin, cb) => {
    if (!origin) return cb(null, true) // webhooks MP, curl, server-to-server
    if (
      CORS_LEALTECK.test(origin) ||
      CORS_FIREBASE.test(origin) ||
...
app.post('/registro', async (req, res) => {
  ...
  try {
    ...
    const registro = {
      ...
      // Referidos — opcional: ID del negocio que lo invitó
      ...(referidoPor ? { referidoPor: String(referidoPor).slice(0, 40) } : {}),
    }
...
async function activarNegocio({ registro, registroId, sub }) {
  ...
  // ── Referidos — si vino con código de referido, registrar y notificar ──
  if (registro.referidoPor && registro.referidoPor !== businessId) {
    procesarReferido({
      referidorId: registro.referidoPor,
      nuevoNegocioId: businessId,
      nuevoNegocioNombre: registro.nombre,
      nuevoNegocioPlan: registro.planClave || registro.plan,
    }).catch(e => console.warn('[referido] Error procesando referido:', e))
  }
...
// ── Helper: registrar referido y notificar al referidor ────────────────
async function procesarReferido({ referidorId, nuevoNegocioId, nuevoNegocioNombre, nuevoNegocioPlan }) {
  const ahora = admin.firestore.FieldValue.serverTimestamp()

  // Verificar que el negocio referidor exista y esté activo
  const referidorDoc = await db.collection('businesses').doc(referidorId).get()
  if (!referidorDoc.exists) {
    console.warn('[referido] Referidor no encontrado:', referidorId)
    return
  }
  const referidor = referidorDoc.data()

  // Guardar el referido en platform/referidos/items
  await db.collection('platform').doc('referidos').collection('items').add({
    referidorId,
    referidorNombre: referidor.nombre || referidorId,
    nuevoNegocioId,
    nuevoNegocioNombre,
    nuevoNegocioPlan,
    estado:   'pendiente_credito',  // Ivan lo aplica manualmente o se automatiza
    creadoEn: ahora,
  })

  // Incrementar contador de referidos en el negocio referidor
  await db.collection('businesses').doc(referidorId).update({
    referidosActivos: admin.firestore.FieldValue.increment(1),
  })

  // Notificar a Ivan para que aplique el crédito
  try {
    await resend.emails.send({
      from:    FROM_EMAIL,
      to:      IVAN_EMAIL,
      subject: `🤝 Nuevo referido: ${referidor.nombre || referidorId} → ${nuevoNegocioNombre}`,
      html: `
        <div style="font-family:system-ui,sans-serif;max-width:520px;margin:0 auto;color:#1c1410">
          <h2 style="color:#C2522F">Referido exitoso 🎉</h2>
          <table style="width:100%;border-collapse:collapse">
            <tr><td style="padding:6px 0;color:#8b7355">Referidor</td><td><strong>${referidor.nombre || referidorId}</strong> (${referidorId})</td></tr>
            <tr><td style="padding:6px 0;color:#8b7355">Negocio nuevo</td><td><strong>${nuevoNegocioNombre}</strong> (${nuevoNegocioId})</td></tr>
            <tr><td style="padding:6px 0;color:#8b7355">Plan del nuevo</td><td>${nuevoNegocioPlan}</td></tr>
          </table>
          <p style="margin-top:20px;background:#f0fdf4;padding:12px 16px;border-radius:8px;color:#15803d">
            ✅ El negocio referidor merece 1 mes gratis. Aplicar crédito manualmente en MercadoPago
            o configurar descuento en la próxima renovación de <strong>${referidorId}</strong>.
          </p>
        </div>
      `,
    })
  } catch (e) {
    console.warn('[referido] Error enviando email notificación:', e)
  }

  // Email al referidor: gracias, tu crédito está en camino
  if (referidor.adminEmails?.[0]) {
    try {
      await resend.emails.send({
        from:    FROM_EMAIL,
        to:      referidor.adminEmails[0],
        subject: '¡Tu referido se unió a Lealteck! 🎉',
        html: `
          <div style="font-family:system-ui,sans-serif;max-width:520px;margin:0 auto;color:#1c1410">
            <h2 style="color:#C2522F">¡Tu referido activó Lealteck!</h2>
            <p><strong>${nuevoNegocioNombre}</strong> acaba de activar su plan en Lealteck gracias a tu recomendación.</p>
            <p style="background:#f0fdf4;padding:12px 16px;border-radius:8px;color:#15803d">
              🎁 Como agradecimiento, vamos a aplicarte <strong>1 mes gratis</strong> en tu próxima renovación.
              Te confirmaremos por email cuando esté aplicado.
            </p>
            <p style="color:#8b7355;font-size:0.85rem">
              Seguís acumulando: por cada negocio que referas y active su plan, sumás 1 mes gratis adicional.
            </p>
          </div>
        `,
      })
    } catch (e) {
      console.warn('[referido] Error enviando email al referidor:', e)
    }
  }

  console.log('[referido] Procesado:', referidorId, '→', nuevoNegocioId)
}
